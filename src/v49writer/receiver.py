...
from . import vita49
from .capture import CaptureSession, StreamFramer
...
def receive_udp(session: CaptureSession, host: str, port: int,
                duration: Optional[float] = None,
                stop: Optional[Callable[[], bool]] = None,
                on_ready: Optional[Callable[[], None]] = None) -> None:
    """Receive VRT packets over UDP until the session is done, the
    duration elapses, or ``stop()`` returns True. If ``host`` is a
    ...
    try:
        while not session.done and not _expired(deadline):
            if stop and stop():
                break
            try:
...
def receive_tcp(session: CaptureSession, host: str, port: int,
                connect: bool = False,
                duration: Optional[float] = None,
                stop: Optional[Callable[[], bool]] = None,
                on_ready: Optional[Callable[[], None]] = None) -> None:
